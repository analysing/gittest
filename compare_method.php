...
foreach ($s_data as $k => $v) {
    if (!in_array($v['name'], $t_name_arr)) {
        $mg_id = $mg_max_id + $i;
        $mg_sql = "insert into $mg_tbl (`lottery_id`, `name`, `group_tag`, `is_merge`, `description`, `sort`, `status`, `ts`) values ($t_lottery_id, '{$v['name']}', '{$v['group_tag']}', '{$v['is_merge']}', '{$v['description']}', '{$v['sort']}', '{$v['status']}', '{$v['ts']}');";
        ...
        $m_sql_prefix = "insert into $m_tbl (`lottery_id`, `mg_id`, `name`, `cname`, `description`, `max_comb`, `max_money`, `levels`, `is_lock`, `expands`, `field_def`, `can_input`, `status`, `method_property`, `sort`, `ts`) values ";
        foreach ($s_m_data as $kk => $vv) {
            $m_id = $m_max_id + $j;
            $m_sql = "{$m_sql_prefix}($t_lottery_id, 'mg_idivyivy', '{$vv['name']}', '{$vv['cname']}', '{$vv['description']}', '{$vv['max_comb']}', '{$vv['max_money']}', '{$vv['levels']}', '{$vv['is_lock']}', '{$vv['expands']}', '{$vv['field_def']}', '{$vv['can_input']}', '{$vv['status']}', '{$vv['method_property']}', '{$vv['sort']}', '{$vv['ts']}');";
            ...
            foreach ($s_p_data as $kkk => $vvv) {
                $p_sql .= "($t_lottery_id, $pg_id, 'm_idbillbill', {$vvv['level']}, {$vvv['prize']}, {$vvv['top_rebate']}, '{$vvv['ts']}'),";
            }
            $p_sql = rtrim($p_sql, ',') .';';
